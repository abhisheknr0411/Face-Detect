...
#now we create a new method to create a dataset 
#of people to train the code to recognize the people trained into it
def generate_dataset(img, id, img_id):
    cv2.imwrite("abhishek/user."+str(id)+'.'+str(img_id)+'.jpg' , img)
...
def draw_boundary(img, classifier, scaleFactor, minNeighbors, color, text, clf):
    ...
    for (x,y,w,h) in features:
        ...
        if id==1:
            ...
            cv2.putText(img, "Abhishek", (x, y-4), cv2.FONT_HERSHEY_COMPLEX, 0.8, color,1, cv2.LINE_AA)
        coords=[x,y,w,h]
